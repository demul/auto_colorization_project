import numpy as np
import cv2
# import matplotlib.pyplot as plt


def save_gamut(quantization_grid):
    #################################################
    # I get CIE-Lab space from uint8-type RGB space
    # Because opencv transform float32-type RGB space to CIE-Lab space in range :
    # 0.0 <= L <= 100.0
    # -86.1813 <= a <= 98.2352
    # -107.862 <= b <= 94.4758
    #
    # The negative values of a, b make gamut-tablizing(tablize_gamut()) complex.
    # Because index of np.array must be positive or 0
    # So, i transform uint8-type RGB space to CIE-Lab space in range :
    # 0 <= L <= 255
    # 42 <= a <= 226
    # 20 <= b <= 223
    #
    # For simplifying gamut-tablizing

    #################################################
    # My quantization method is simple.
    # Floor-divide values of a, b with quantization_grid.
    # And multiply it again with quantization_grid.
    # Quantization_grid is hyperparameter.
    # I only experiment the grid-setting on 10
    # A total of 262 a, b pairs are in gamut.
    set_pairs = set()
    for red in range(256):
        for green in range(256):
            for blue in range(256):
                im = np.array((blue, green, red), np.uint8).reshape(1, 1, 3)
                im = cv2.cvtColor(im, cv2.COLOR_BGR2LAB)

                ## quantize
                a = (im[0, 0, 1] // quantization_grid) * quantization_grid
                b = (im[0, 0, 2] // quantization_grid) * quantization_grid

                set_pairs.add((a, b))

    set_pairs = list(set(set_pairs))  # deduplication
    filename_gamut = 'gamut_grid%d_class%d_uint8.npy' % (quantization_grid, len(set_pairs))
    np.save(filename_gamut, np.array(list(set_pairs)))
    ## visualization
    # plt.scatter([x[0] for x in gamut_quantized], [x[1] for x in gamut_quantized])
    # plt.xlabel('a')
    # plt.ylabel('b')
    # plt.show()

    return filename_gamut


def tablize_gamut(gamut):
    #################################################
    # The index of this table is values of a, b divided by 10
    gamut = gamut // 10
    len_gamut = gamut.shape[0]
    max_ab = np.max(gamut, axis=0)

    #################################################
    ## max_ab + 2 = max_ab + 1 + 1
    ## + 1 (because max index of np.array is width - 1(or height - 1))
    ## + 1 (for padding right, bottom side of the table by 1 for simplify soft encoding)
    table_gamut = np.zeros([max_ab[0] + 2, max_ab[1] + 2], dtype=np.uint32)

    for i in range(len_gamut):
        #################################################
        ## Value of index start from 1,
        ## Because 0 is a value for index-pairs which is not in gamut.
        table_gamut[tuple(gamut[i])] = i + 1

    return table_gamut


def labels2encoding(num_colors, label, label_up, label_left, label_down, label_right, variance, quantization_grid = 10):
    #################################################
    # Must subtract all labels by 1.
    # Because i use zero to denote 'This color is out of gamut'
    label -= 1
    label_down -= 1
    label_left -= 1
    label_right -= 1
    label_up -= 1

    encoding = np.zeros([num_colors], dtype=np.float32)

    # get gaussian softening parameter(probability)
    center_prob = (1 / (2 * np.pi * (variance ** 2)))
    nearest_prob = (1 / (2 * np.pi * (variance ** 2))) * np.exp(-(2 * (quantization_grid ** 2)) / (2 * (variance ** 2)))
    sum_probs = center_prob + 4 * nearest_prob
    center_prob = center_prob / sum_probs
    nearest_prob = nearest_prob / sum_probs

    # set center probability
    encoding[int(label)] += center_prob

    # set nearest probabilities
    ## If some nearest label has 0 value, it mean that the center color is at the edge of gamut.
    ## So, the out-of-ranged color's probability must be 0, and added into the center color's probability.
    if label_up == -1:
        encoding[int(label)] += nearest_prob
    else:
        encoding[int(label_up)] = nearest_prob

    if label_down == -1:
        encoding[int(label)] += nearest_prob
    else:
        encoding[int(label_down)] = nearest_prob

    if label_left == -1:
        encoding[int(label)] += nearest_prob
    else:
        encoding[int(label_left)] = nearest_prob

    if label_right == -1:
        encoding[int(label)] += nearest_prob
    else:
        encoding[int(label_right)] = nearest_prob

    return encoding


def get_softencoding_mapping(num_colors, table_gamut, variance):
    table_mapping = dict()
    h = table_gamut.shape[0]
    w = table_gamut.shape[1]

    for y in range(h):
        for x in range(w):
            label = table_gamut[y, x]

            if label != 0: # The color whose label is 0 is out of gamut.
                # get nearest 4 color
                label_up = table_gamut[y - 1, x]
                label_left = table_gamut[y, x - 1]
                label_down = table_gamut[y + 1, x] # This is why I pad right, bottom side of gamut's table.
                label_right = table_gamut[y, x + 1] # For avoiding index out of range.

                encoding = labels2encoding(num_colors, label, label_up, label_left, label_down, label_right, variance)

                table_mapping[label] = encoding

    return table_mapping

class Decoder:
    def __init__(self, table_gamut, temperature=0.38):
        self.table_gamut = table_gamut
        self.table_gamut_inverse = self.get_inverse_table(table_gamut)
        self.temperature = temperature


    def softmax_temperature_scaled(self, prob):
        #################################################
        # Reformulation to avoid log(0)
        #   exp(log(probability) / T) / sum(exp(log(probability) / T))
        # = probability^(1/T) / sum(probability^(1/T))
        prob_scaled = (prob ** (1 / self.temperature)) / np.sum(prob ** (1 / self.temperature))
        return prob_scaled


    def get_inverse_table(self, table_gamut):
        table_gamut_inverse = dict()

        for i in range(1, table_gamut.max()+ 1):
            a = np.where(table_gamut == i)[0][0]
            b = np.where(table_gamut == i)[1][0]
            table_gamut_inverse[i] = np.array([a, b], dtype=np.uint8)

        return table_gamut_inverse


    def softencoding2ab(self, batch_softencoding):
        # batch_softencoding : [None, 56, 56, 262]
        batch_size = batch_softencoding.shape[0]
        h = batch_softencoding.shape[1]
        w = batch_softencoding.shape[2]

        # output must be : [None, 56 * 4, 56 * 4, 262]
        batch_ab = np.empty([batch_size, h * 4, w * 4, 2], dtype=np.uint8)

        for idx, softencoding in enumerate(batch_softencoding):
            img_ab = np.empty([h, w, 2], dtype=np.uint8)
            for y in range(h):
                for x in range(w):
                    img_ab[y, x] = self.annealed_mean_decode(softencoding[y, x])

            # upscale ab image
            batch_ab[idx] = cv2.resize(img_ab, (h * 4, w * 4), cv2.INTER_LINEAR)

        return batch_ab

    def annealed_mean_decode(self, encoding):
        # batch_softencoding : [262,]
        encoding_scaled = self.softmax_temperature_scaled(encoding)
        annealed_mean = np.zeros([2, ], dtype=np.float32)

        for idx, prob in enumerate(encoding_scaled):
            ab = self.table_gamut_inverse[idx + 1] * 10
            annealed_mean += ab * prob

        return annealed_mean.astype(np.uint8)

    def encoding2bgr(self, batch_L, batch_softencoding):
        # batch_L : [None, 224, 224, 1]
        # batch_softencoding : [None, 56, 56, 262]
        # batch_bgr : [None, 224, 224, 3]
        batch_size = batch_L.shape[0]
        h = batch_L.shape[1]
        w = batch_L.shape[2]

        # soft encoding to ab
        batch_ab = self.softencoding2ab(batch_softencoding)
        # concat Lab
        batch_Lab = np.concatenate([batch_L, batch_ab], axis=3).astype(np.uint8)
        # convert Lab to bgr
        batch_bgr = np.empty([batch_size, h, w, 3], dtype=np.uint8)
        for idx, img in enumerate(batch_Lab):
            batch_bgr[idx] = cv2.cvtColor(batch_Lab[idx], cv2.COLOR_LAB2BGR)

        return batch_bgr
