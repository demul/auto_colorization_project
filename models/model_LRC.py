...
class model:
    ...
    def generator(self, x, isTrain=True, keep_prob=0.5, reuse=None):
        # x = condition [None, 256, 256, 1]
        # pix2pix에서 drop-out을 deterministic한 결과를 얻기 위한 일종의 noise로 사용하기 위해 학습과 예측에 모두 사용
        with tf.variable_scope('generator', reuse=reuse):
            ...
            # drop-out은 일종의 noise로 사용하기 위해 학습과 예측에 모두 사용
            ...
            self.uW6 = tf.get_variable("G_11conv", shape = [1, 1, 512, 3], dtype=np.float32,
                                      initializer = tf.random_normal_initializer(0, 0.01))
            self.uL6 = tf.nn.conv2d(self.uL5, self.uW6, strides=[1, 1, 1, 1], padding='SAME') # [None, 32, 32, 3]

            return tf.nn.tanh(self.uL6) * 255

    def discriminator(self, x, unknown, isTrain=True, reuse=None):
        x = tf.concat([x, unknown], axis=3)
        # x = condition + label [None, 32, 32, 1 + 3]

        with tf.variable_scope('discriminator', reuse=reuse):
            # 1st conv
            self.d_W1 =tf.get_variable("D_conv_w1", shape = [4, 4, 4, 64], dtype=np.float32,
                                       initializer = tf.random_normal_initializer(0, 0.01))
            self.d_L1 = tf.nn.conv2d(x, self.d_W1, strides=[1, 2, 2, 1], padding='SAME') # [None, 16, 16, 64]
            ...
            self.d_W2 = tf.get_variable("D_conv_w2", shape = [4, 4, 64, 128], dtype=np.float32,
                                        initializer=tf.random_normal_initializer(0, 0.01))
            self.d_L2 = tf.nn.conv2d(self.d_L1, self.d_W2, strides=[1, 2, 2, 1], padding='SAME') # [None, 8, 8, 128]
            ...
            self.d_W3 = tf.get_variable("D_conv_w3", shape=[1, 1, 128, 256], dtype=np.float32,
                                        initializer=tf.random_normal_initializer(0, 0.01))
            self.d_L3 = tf.nn.conv2d(self.d_L2, self.d_W3, strides=[1, 1, 1, 1], padding='SAME')  # [None, 8, 8, 256]
            ...
            self.d_W4 = tf.get_variable("D_conv_w4", shape=[4, 4, 256, 512], dtype=np.float32,
                                        initializer=tf.random_normal_initializer(0, 0.01))
            self.d_L4 = tf.nn.conv2d(self.d_L3, self.d_W4, strides=[1, 1, 1, 1], padding='SAME')  # [None, 8, 8, 512]
            ...
            self.d_W5 = tf.get_variable("D_11conv", shape=[1, 1, 512, 1], dtype=np.float32,
                                        initializer=tf.random_normal_initializer(0, 0.01))
            self.d_L5 = tf.nn.conv2d(self.d_L4, self.d_W5, strides=[1, 1, 1, 1], padding='SAME')  # [None, 8, 8, 1]
