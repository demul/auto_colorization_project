...
class CGAN:
    def __init__(self, input_size, lr = 0.0002, smoothing_factor=0.2, input_dir = "yumi_cell", result_dir = 'LRC_results'):
        self.lr = lr
    ...
    def make_train_graph(self, edge_img, GT_img,is_training, keep_prob, gpu_num, split_num):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            train_op_d = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(tf.reduce_mean(total_D_loss),
                                                                                          var_list=vars_d,
                                                                                          colocate_gradients_with_ops=True)
            train_op_g = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(tf.reduce_mean(target_G_loss),
                                                                                          var_list=vars_g,
                                                                                          colocate_gradients_with_ops=True)
            # train_op_g = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(tf.reduce_mean(total_L1_loss),
            #                                                                               var_list=vars_g,
            #                                                                               colocate_gradients_with_ops=True)

        return train_op_d, train_op_g, total_D_loss, total_G_loss, total_Generated_img
        # return train_op_d, train_op_g, total_D_loss, total_L1_loss, total_Generated_img
    ...
    def inference(self, gpu_num, split_num):
        ...
        for iter in range(300 // self.input_size):
            ...
            images_result_path = os.path.join(self.result_dir, 'Fixed_noise_results', 'inference%4d.png' % (iter + 1))
    ...
    def make_data_for_BD(self, gpu_num, split_num, start = 0, end = 7380): #BD를 학습시키려면 LRC의 결과물이 필요하다.
        #load image loader
        img_loader = data_loader.Loader(dir = self.input_dir)
        ...
        images_result_dir = os.path.join(self.input_dir, 'LRC')
